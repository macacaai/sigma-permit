...
export default function PlansPage() {
  ...
  const loadPlans = async (page = 1, size = pageSize) => {
    try {
      ...
      const response = await planApi.getPlans(page, size);
  ...
  const handlePageSizeChange = (size: number) => {
    loadPlans(1, size); // Reset to first page when changing page size
  };

  const filteredPlans = plans.filter(plan =>
    plan.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    (plan.product_name && plan.product_name.toLowerCase().includes(searchQuery.toLowerCase())) ||
    plan.billing_interval.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const confirmDelete = async () => {
    ...
    try {
      await planApi.deletePlan(selectedPlan.id);
  ...
  const handleFormSuccess = () => {
    ...
    setTimeout(() => loadPlans(currentPage), 500);
  };
    ...
    <ProtectedRoute>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-50">Plans</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-2">Manage subscription plans for products</p>
          </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-gray-50 mb-2">No plans found</h3>
                <p className="text-gray-600 dark:text-gray-400 mb-4">
                  {searchQuery ? 'Try adjusting your search terms.' : 'Get started by creating your first plan.'}
                </p>
                {!searchQuery && (
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredPlans.map((plan) => (
                    <tr key={plan.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="font-medium text-gray-900 dark:text-gray-50">{plan.name}</div>
                      </td>
        ...
        <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Add a new subscription plan for a product.
              </DialogDescription>
            </DialogHeader>
            <PlanForm
                ...
                <div className="flex-1">
                  <div className="mt-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                    <p className="text-sm text-yellow-800 dark:text-yellow-200">
                      This will permanently delete the plan and may affect existing subscriptions.
                    </p>
                  </div>
                </div>
              </div>
