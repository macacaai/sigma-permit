...
// When served through Caddy, use relative URLs for API calls
...
export const tenantApi = {
  ...
  getTenantLicenses: (tenantId: string, page = 1, size = 10) =>
    api.get<ApiResponse<any>>(`/tenants/${tenantId}/licenses?page=${page}&size=${size}`),

  // Get tenant's subscriptions
  getTenantSubscriptions: (tenantId: string, page = 1, size = 10) =>
    api.get<ApiResponse<any>>(`/tenants/${tenantId}/subscriptions?page=${page}&size=${size}`),

  // Get tenant's products (products associated with tenant's subscriptions)
  getTenantProducts: (tenantId: string, page = 1, size = 10) =>
    api.get<ApiResponse<any>>(`/tenants/${tenantId}/products?page=${page}&size=${size}`),

  // Get tenant's plans (plans associated with tenant's subscriptions)
  getTenantPlans: (tenantId: string, page = 1, size = 10) =>
    api.get<ApiResponse<any>>(`/tenants/${tenantId}/plans?page=${page}&size=${size}`),

  // Get tenant's features (features in tenant's subscription plans)
  getTenantFeatures: (tenantId: string, page = 1, size = 10) =>
    api.get<ApiResponse<any>>(`/tenants/${tenantId}/features?page=${page}&size=${size}`),
};
...
export const subscriptionApi = {
  ...
  // Get subscription entitlements with pagination
  getSubscriptionEntitlements: (subscriptionId: string, page = 1, size = 10) => {
    const params = new URLSearchParams({ page: page.toString(), size: size.toString() });
    return api.get<ApiResponse<any>>(`/subscriptions/${subscriptionId}/entitlements?${params.toString()}`);
  },

  // Create subscription entitlement
  createSubscriptionEntitlement: (subscriptionId: string, data: any) =>
    api.post<any>(`/subscriptions/${subscriptionId}/entitlements`, data),

  // Update subscription entitlement
  updateSubscriptionEntitlement: (subscriptionId: string, featureId: string, data: any) =>
    api.put<any>(`/subscriptions/${subscriptionId}/entitlements/${featureId}`, data),

  // Delete subscription entitlement
  deleteSubscriptionEntitlement: (subscriptionId: string, featureId: string) =>
    api.delete<any>(`/subscriptions/${subscriptionId}/entitlements/${featureId}`),
};

// Master Key API
export const masterKeyApi = {
  // Get all master keys (admin only)
  getMasterKeys: () =>
    api.get<MasterKey[]>('/licenses/master-keys'),

  // Rotate master key
  rotateMasterKey: () =>
    api.post<any>('/licenses/master-keys/rotate'),

  // Override master key
  overrideMasterKey: (publicKey: string, privateKey: string) =>
    api.post<any>('/licenses/master-keys/override', { public_key: publicKey, private_key: privateKey }),

};

// Applications API (OAuth2 client applications)
export const applicationsApi = {
  // Get all applications with pagination and filtering
  getApplications: (params?: Record<string, any>) => {
    const queryParams = params ? new URLSearchParams(params).toString() : '';
    return api.get<ApiResponse<any>>(`/v1/applications${queryParams ? `?${queryParams}` : ''}`);
  },

  // Get application by ID
  getApplication: (id: string) =>
    api.get<any>(`/v1/applications/${id}`),

  // Create application
  createApplication: (data: any) =>
    api.post<any>('/v1/applications', data),

  // Update application
  updateApplication: (id: string, data: any) =>
    api.put<any>(`/v1/applications/${id}`, data),

  // Delete application
  deleteApplication: (id: string) =>
    api.delete<any>(`/v1/applications/${id}`),

  // Rotate client secret
  rotateSecret: (id: string) =>
    api.post<any>(`/v1/applications/${id}/secret`),

  // Get authorized users for application
  getAuthorizedUsers: (id: string) =>
    api.get<any[]>(`/v1/applications/${id}/authorized-users`),

  // Grant user access to application
  grantUserAccess: (id: string, userId: string, scope?: string) =>
    api.post<any>(`/v1/applications/${id}/authorized-users`, { user_id: userId, scope }),

  // Revoke user access from application
  revokeUserAccess: (id: string, userId: string) =>
    api.delete<any>(`/v1/applications/${id}/authorized-users/${userId}`),
};
...
export const sessionsApi = {
  ...
  getSessions: (params?: Record<string, any>) => {
    const queryParams = params ? new URLSearchParams(params).toString() : '';
    return api.get<ApiResponse<any>>(`/v1/sessions${queryParams ? `?${queryParams}` : ''}`);
  },

  // Get session by ID
  getSession: (id: string) =>
    api.get<any>(`/v1/sessions/${id}`),

  // Terminate session
  terminateSession: (id: string) =>
    api.delete<any>(`/v1/sessions/${id}`),

  // Terminate all sessions
  terminateAllSessions: () =>
    api.delete<any>('/v1/sessions'),

  // Refresh session
  refreshSession: (id: string) =>
    api.put<any>(`/v1/sessions/${id}/refresh`),

  // Lock session
  lockSession: (id: string) =>
    api.post<any>(`/v1/sessions/${id}/lock`),

  // Unlock session
  unlockSession: (id: string) =>
    api.post<any>(`/v1/sessions/${id}/unlock`),

  // Require MFA for session
  requireMfa: (id: string) =>
    api.post<any>(`/v1/sessions/${id}/require-mfa`),
};
